...
public:
    ...
    bool push(const T& value, std::stop_token st = {}) { return emplace_impl(value, st); }
    bool push(T&& value, std::stop_token st = {}) { return emplace_impl(std::move(value), st); }

    template <class... Args>
    bool emplace(std::stop_token st, Args&&... args) {
...
private:
    template <typename U>
    bool emplace_impl(U&& value, std::stop_token st) {
#ifndef PLATFORM_FAILURE_RACE
        std::unique_lock lock(mutex_);
#else
        std::unique_lock lock(dummy_mutex_);
