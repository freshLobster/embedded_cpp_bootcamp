...
**Mission:** Environment, CPU/GPU parity, RAII CUDA buffer for mission-critical robotics/autonomy software. Self-contained; assume only this folder + repo tooling.
...
3. Run this module's focused tests (filter): `ctest --preset dev -R Module11_cuda_basics` (will pass once you add/enable tests as instructed in exercises).
4. Code lives in repo root paths noted in each exercise; stage work-in-progress artifacts under `modules/11_cuda_basics_cuda_basics/artifacts/`.
...
## Grade this module
- Run: python3 tools/grader/grade.py --module modules/11_cuda_basics
- JSON: python3 tools/grader/grade.py --module 11 --json
- Artifacts expected in modules/11_cuda_basics/artifacts/ per exercises.
- CUDA-dependent checks require `--enable-cuda`; otherwise they are skipped with partial credit.
