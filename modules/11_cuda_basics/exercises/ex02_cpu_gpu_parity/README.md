# Module 11_cuda_basics - ex02_cpu_gpu_parity

## 1) Title + Mission
Mission: CPU/GPU parity. You will use a tiny, isolated codebase to practice the build/test/grade loop in a deterministic way.
...
- **CUDA stub path:** `exercise_cpu()` is always available.
- **CUDA path:** define `ENABLE_CUDA` via `-DENABLE_CUDA=ON`. If `find_package(CUDAToolkit)` succeeds, the macro is defined and `exercise()` uses `exercise_gpu()`.
...
Optional (CUDA toolkit for GPU path):
```
...
- CUDA Toolkit installed (CMake can find CUDAToolkit)

No CUDA path:
- Build without `-DENABLE_CUDA=ON`.
- The code uses `exercise_cpu()` and skips GPU-specific paths.
...
- Learner attempt:
  `python3 ../../../../tools/grader/grade.py --exercise ../../../../modules/11_cuda_basics/exercises/ex02_cpu_gpu_parity`
- Solution check:
  `python3 ../../../../tools/grader/grade.py --exercise ../../../../modules/11_cuda_basics/exercises/ex02_cpu_gpu_parity --use-solution`
- High-level scoring:
