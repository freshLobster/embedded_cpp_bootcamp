...
**Mission:** QEMU/Renode for deterministic repro + CI harness for mission-critical robotics/autonomy software. Self-contained; assume only this folder + repo tooling.
...
3. Run this module's focused tests (filter): `ctest --preset dev -R Module10_sim` (will pass once you add/enable tests as instructed in exercises).
4. Code lives in repo root paths noted in each exercise; stage work-in-progress artifacts under `modules/10_sim_simulation/artifacts/`.
...
## Grade this module
- Run: python3 tools/grader/grade.py --module modules/10_sim
- JSON: python3 tools/grader/grade.py --module 10 --json
- Artifacts expected in modules/10_sim/artifacts/ per exercises.
