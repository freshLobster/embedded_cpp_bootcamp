...
**Mission:** Queues, deadlocks, races, clean shutdown for mission-critical robotics/autonomy software. Self-contained; assume only this folder + repo tooling.
...
3. Run this module's focused tests (filter): `ctest --preset dev -R Module03_concurrency` (will pass once you add/enable tests as instructed in exercises).
4. Code lives in repo root paths noted in each exercise; stage work-in-progress artifacts under `modules/03_concurrency_concurrency/artifacts/`.
...
## Grade this module
- Run: python3 tools/grader/grade.py --module modules/03_concurrency
- JSON: python3 tools/grader/grade.py --module 03 --json
- Artifacts expected in modules/03_concurrency/artifacts/ per exercises.
