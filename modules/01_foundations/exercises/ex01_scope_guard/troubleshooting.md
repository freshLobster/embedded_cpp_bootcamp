# Troubleshooting - 01_foundations ex01_scope_guard

## Common failures

1) **Build fails due to TODO placeholder**
   - Symptom: compiler error referencing `#error TODO_implement_exercise`.
   - Fix: remove the `#error` line after you implement a RAII scope guard that runs a cleanup callback exactly once on scope exit, with a dismiss option and move-only semantics.
   - Verify: `cmake --build build_learner` succeeds.

2) **Tests fail with assertion**
   - Symptom: `Assertion 'exercise() == 0' failed` or similar.
   - Fix: inspect `exercise()` and ensure it enforces the required behavior for a RAII scope guard that runs a cleanup callback exactly once on scope exit, with a dismiss option and move-only semantics.
